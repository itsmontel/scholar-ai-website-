...
/* ═══════════════════════════════════════════════════════════════
   100-level system — XP formula: 10*(L-1)^2 + 20*(L-1)
   Tier names rotate every 5 levels with Roman numeral suffixes
   ═══════════════════════════════════════════════════════════════ */
const LEVEL_TIER_NAMES = [
  'Scholar Seedling',     // 1–5
  'Curious Cat',          // 6–10
  'Knowledge Keeper',     // 11–15
  'Brain Explorer',       // 16–20
  'Wisdom Warrior',       // 21–25
  'Study Sage',           // 26–30
  'Academic Ace',         // 31–35
  'Genius Guide',         // 36–40
  'Master Mind',          // 41–45
  'Supreme Scholar',      // 46–50
  'Diamond Brain',        // 51–55
  'Platinum Thinker',     // 56–60
  'Cosmic Scholar',       // 61–65
  'Legendary Learner',    // 66–70
  'Mythic Mind',          // 71–75
  'Eternal Scholar',      // 76–80
  'Celestial Sage',       // 81–85
  'Omniscient Oracle',    // 86–90
  'Transcendent Genius',  // 91–95
  'Supreme Grandmaster',  // 96–100
];
const ROMAN = ['I', 'II', 'III', 'IV', 'V'];

function levelXP(l: number): number {
  if (l <= 1) return 0;
  return Math.floor(10 * (l - 1) ** 2 + 20 * (l - 1));
}

const LEVELS: LevelInfo[] = Array.from({ length: 100 }, (_, i) => {
  const level = i + 1;
  const tierIdx = Math.min(Math.floor(i / 5), LEVEL_TIER_NAMES.length - 1);
  const sub = i % 5;
  return {
    level,
    name: `${LEVEL_TIER_NAMES[tierIdx]} ${ROMAN[sub]}`,
    minXP: levelXP(level),
    maxXP: level === 100 ? Infinity : levelXP(level + 1),
  };
});
...
export const BADGES: Badge[] = [
  ...
  { id: 'dedicated_scholar', name: 'Dedicated Scholar', creatureName: 'Devotion', description: '6 months as a paid subscriber', xp: 500, category: 'subscription', rarity: 'legendary', condition: (s) => s.is_paid_user && monthsSincePaid(s) >= 6, conditionText: '6 months as paid subscriber' },
