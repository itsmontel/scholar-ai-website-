 ...
 *
 * 3-step recovery flow shown when a paying user clicks "Cancel
 * subscription":
 *   1. Pause for 30 days?
 *   2. Stay for 50% off next month?
 *   3. Confirm cancel
 *
 * For users still in their free trial we skip steps 1 + 2 (pause
 * makes no sense during a trial, and the 50%-off coupon would
 * collide with the NEWCUSTOMER discount already attached to their
 * first cycle) and open straight at step 3.
 *
...
interface CancelRetentionModalProps {
  ...
  onMutate?: () => void;
  /** User's current Stripe subscription_status. When 'trialing' the modal opens at step 3 directly. */
...
export default function CancelRetentionModal({
  ...
  // For trial users, jump straight to the confirm-cancel step.
  // For active subscribers, walk the full pause → discount → confirm flow.
  const isTrialing = subscriptionStatus === 'trialing';
  const initialStep: RetentionStep = isTrialing ? 'confirm' : 'pause';
  ...
  const acceptDiscount = async () => {
    ...
    try {
      ...
      setSuccessMsg('50% off applied to your next invoice. Enjoy the half-price month on us.');
      setStep('success');
      onMutate?.();
    } catch (e) {
  ...
  // The 3-step progress dots only show for the full retention flow.
  // Trial users get a single-screen confirm, so the dots would
  // misleadingly suggest there's a multi-step process.
  const showStepDots = !isTrialing && step !== 'success';
            ...
            <div className="text-center">
              ...
              <div className="mt-5 space-y-2.5">
                <button
                  ...
                  className="w-full py-3 px-4 rounded-2xl bg-[#58CC02] hover:bg-[#46A302] disabled:opacity-60 disabled:cursor-not-allowed text-white text-[13px] sm:text-sm font-extrabold uppercase tracking-wide border-2 border-b-4 border-[#46A302] active:border-b-2 active:translate-y-0.5 transition-all"
                >
                  {busy ? 'Applying…' : 'Yes, apply 50% off'}
                </button>
                <button
                  type="button"
