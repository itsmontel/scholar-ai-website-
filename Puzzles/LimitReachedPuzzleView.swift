...
struct LimitReachedPuzzleView: View {
    ...
    @AppStorage("puzzleHardModeEnabled") private var hardModeEnabled = false
    ...
    private func startRandomPuzzle() {
        ...
        // Generate puzzles once and store them (with hard mode support)
        if let puzzleType = selectedPuzzleType {
            switch puzzleType {
            case .sudoku:
                currentSudokuPuzzle = SudokuPuzzle.generate(hardMode: hardModeEnabled)
            case .memory:
                currentMemoryGame = MemoryGame.generate(hardMode: hardModeEnabled)
            case .pattern:
                currentPatternSequence = PatternSequence.generate(hardMode: hardModeEnabled)
            case .jigsaw:
                currentJigsawPuzzle = JigsawPuzzle.generate(hardMode: hardModeEnabled)
            }
        }
