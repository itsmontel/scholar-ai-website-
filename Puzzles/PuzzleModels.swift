...
struct SudokuPuzzle {
    ...
    static func generate(hardMode: Bool = false) -> SudokuPuzzle {
        ...
        // Normal mode: remove 8-12 cells, Hard mode: remove 16-20 cells (much fewer starting numbers)
        var puzzle = solution.map { $0.map { Optional($0) } }
        let cellsToRemove = hardMode ? Int.random(in: 16...20) : Int.random(in: 8...12)
        var removed = 0
...
struct MemoryGame {
    ...
    var gridColumns: Int {
        // Normal: 4x4, Hard: 4x5 (4 across, 5 down)
        4
    }
    
    static func generate(hardMode: Bool = false) -> MemoryGame {
        // Normal mode: 8 pairs for 4x4 grid (16 cards total)
        // Hard mode: 10 pairs for 4x5 grid (20 cards total)
        let allSymbols = ["🎮", "🎯", "🎪", "🎨", "🎭", "🎸", "🎲", "🎺", "🎹", "🎤"]
        let symbols = hardMode ? allSymbols : Array(allSymbols.prefix(8))
        ...
        return MemoryGame(cards: cards, isHardMode: hardMode)
    }
}
...
struct PatternSequence {
    ...
    static func generate(length: Int = 6, hardMode: Bool = false) -> PatternSequence {
        // Normal mode: 6 elements, Hard mode: 7 elements
        let actualLength = hardMode ? 7 : length
        
        // Generate exactly 'actualLength' random elements
        var elements: [PatternElement] = []
        for _ in 0..<actualLength {
            elements.append(PatternElement.allCases.randomElement()!)
        }
        return PatternSequence(elements: elements, length: actualLength)
    }
...
struct JigsawPuzzle {
    ...
    let gridSize: Int // 3 for normal, 4 for hard mode
    let isHardMode: Bool
    ...
    static func generate(hardMode: Bool = false) -> JigsawPuzzle {
        // Pick a random pet mood image from the 25 available
        let randomImage = allPetImages.randomElement() ?? "dogfullhealth"
        return generate(withImage: randomImage, hardMode: hardMode)
    }
    
    // Generate with a specific image
    static func generate(withImage imageName: String, hardMode: Bool = false) -> JigsawPuzzle {
        // Normal mode: 3x3 grid (9 pieces), Hard mode: 4x4 grid (16 pieces)
        let gridSize = hardMode ? 4 : 3
        let totalPieces = gridSize * gridSize
        
        // Create pieces with proper interlocking edges
        var pieces: [JigsawPiece] = []
        
        // Define edge patterns - adjacent pieces must have matching tab/blank
        let edgePattern: [[JigsawEdge]] = generateEdgePattern(size: gridSize)
        
        for position in 0..<totalPieces {
        ...
        return JigsawPuzzle(imageName: imageName, shuffledPieces: shuffled, gridSize: gridSize, isHardMode: hardMode)
    }
    
    // Generate random edge pattern for bottom edges (determines interlocking)
    private static func generateEdgePattern(size: Int) -> [[JigsawEdge]] {
