...
class PuzzleManager: ObservableObject {
    ...
    func grantExtension(for bundleID: String, puzzleType: PuzzleType) {
        ...
        // Track puzzle type for "Puzzle Variety" achievement
        trackPuzzleTypeForAchievement(puzzleType)
        
        print("✅ Granted \(extensionMinutes) minutes extension for \(bundleID) via \(puzzleType.rawValue) puzzle")
    }
    
    /// Tracks puzzle types solved for the "Puzzle Variety" achievement
    private func trackPuzzleTypeForAchievement(_ puzzleType: PuzzleType) {
        guard let defaults = UserDefaults(suiteName: "group.com.virtupet.screentime") else { return }
        
        var solvedTypes = defaults.array(forKey: "achievement_puzzle_types_solved") as? [String] ?? []
        let puzzleTypeString = puzzleType.rawValue
        
        if !solvedTypes.contains(puzzleTypeString) {
            solvedTypes.append(puzzleTypeString)
            defaults.set(solvedTypes, forKey: "achievement_puzzle_types_solved")
            defaults.synchronize()
            print("🏆 Achievement: New puzzle type '\(puzzleTypeString)' solved (\(solvedTypes.count)/4 types)")
        }
    }
