-- Run in Supabase SQL Editor to add/fix last_active_at.
-- Safe to re-run.
...
-- Check results:
-- SELECT COUNT(*) FILTER (WHERE last_active_at IS NULL) AS still_null,
--        COUNT(*) FILTER (WHERE last_active_at IS NOT NULL) AS has_value
-- FROM users;
