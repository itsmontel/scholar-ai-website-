...
// @route   POST /api/analysis/generate-tower-questions
// @desc    Generate questions for Word Tower from a topic or notes
// @access  Private
router.post('/generate-tower-questions', authenticateToken, async (req, res) => {
  try {
    const { inputType, content } = req.body;
    const userPlan = getEffectivePlan(req);
    const planLimits = subscriptionService.PLAN_LIMITS[subscriptionService.normalizePlanForLimits(userPlan)] || subscriptionService.PLAN_LIMITS.free;
...
// @route   POST /api/analysis/save-word-tower
// @desc    Save a Word Tower game to history for replay
// @access  Private
router.post('/save-word-tower', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const userPlan = getEffectivePlan(req);
    const { questions, title, inputType, sourceText } = req.body;
    ...
    const saved = await aiAnalysisService.saveWordTowerGame(userId, {
      questions,
      title: title || sourceText?.slice(0, 80) || 'Word Tower Game',
      inputType: inputType || 'topic',
      sourceText: sourceText || ''
    }, userPlan);
    ...
    console.error('Save Word Tower error:', error);
    res.status(500).json({ success: false, message: error.message || 'Failed to save game' });
  }
});
...
router.get('/study-pack-usage', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const { plan: dbPlan } = await subscriptionService.getUserSubscriptionDetails(userId);
    const normalizedPlan = subscriptionService.normalizePlanForLimits(dbPlan);
